...
def attempt_dependency_fix(repo: Repo, dep_name: str | None):
    ...
    if manager in {"pip", "python"}:
        return [
            "Python dependency update automation not implemented; use uv/poetry/pip-tools-specific workflow."
        ]
...
def apply_suggestions_best_effort(repo_dir: Path, comments: list[dict]) -> int:
    ...
    if notes:
        (repo_dir / "NOVA_APPLIED_SUGGESTIONS.md").write_text(
            "# Nova captured bot suggestions\n\n" + "\n".join(notes) + "\n",
            encoding="utf-8",
        )
    return count
