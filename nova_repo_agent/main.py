...
from .git_ops import apply_suggestions_best_effort, attempt_dependency_fix, checkout_pr, clone_or_update, commit_all, has_changes, push
...
def load_config(path: str) -> dict[str, Any]:
    ...
    for key in ["scanning","dependabot","pr_comment_edits","merge","exports","nova"]:
        cfg.setdefault(key, {})
...
def authenticated_clone_url(clone_url: str, token: str) -> str:
    return clone_url.replace("https://", f"https://x-access-token:{token}@") if clone_url.startswith("https://") else clone_url
...
def scan_repo(owner, repo_data, cfg, gh, nova, log, workdir, token):
    ...
    log.suggest(name, nova.repo_suggestions(name, repo_inventory(repo_data, alerts, issues, prs, cfg)))
    ...
    repo = clone_or_update(authenticated_clone_url(repo_data["clone_url"], token), repo_dir)
    ...
    for p in prs:
        ...
        try:
            ...
            log.add(Finding.now(owner, name, "pr_suggestion", "warning", f"PR #{prn} suggestion handling failed", action="failed", url=p.get("html_url", ""), details=str(e)))
    ...
    for a in alerts:
        ...
        try:
            ...
            if has_changes(repo):
                ...
                log.add(Finding.now(owner, name, "dependabot_fix", sev, f"No local dependency changes produced for {dep_name}", action="skipped", details="\n".join(outputs)))
        except Exception as e:
            log.add(Finding.now(owner, name, "dependabot_fix", sev, f"Dependency fix failed for {dep_name}", action="failed", details=str(e)))
                ...
                if pr.get("mergeable") is True and pr.get("mergeable_state") in {"clean", "has_hooks", "unstable"}:
                    if cfg["merge"].get("require_green_checks", True):
                        ...
                        if status.get("state") not in {"success", "pending"}:
                            log.add(Finding.now(owner, name, "merge", "info", f"PR #{pr_number} not merged; checks={status.get('state')}", action="skipped", url=pr.get("html_url", "")))
                            continue
                    gh.merge_pr(owner, name, pr_number, cfg["merge"].get("strategy", "squash"))
                    log.add(Finding.now(owner, name, "merge", "info", f"Merged PR #{pr_number}", action="merged", url=pr.get("html_url", "")))
                else:
                    log.add(Finding.now(owner, name, "merge", "info", f"PR #{pr_number} not cleanly mergeable", action="skipped", url=pr.get("html_url", ""), details=f"mergeable={pr.get('mergeable')} state={pr.get('mergeable_state')}"))
            except Exception as e:
                log.add(Finding.now(owner, name, "merge", "warning", f"PR #{pr_number} merge failed", action="failed", url=p.get("html_url", ""), details=str(e)))
...
def main() -> int:
    ...
    if not token:
        raise SystemExit("GITHUB_TOKEN is required. Add it to .env or export it in your shell.")
    owner = cfg["owner"]
    mode = cfg.get("mutation_mode", "dry-run")
    workdir = Path(cfg.get("workdir", ".nova-workdir")).resolve()
    gh = GitHubClient(token)
    nova = NovaClient(os.getenv("NOVA_MODEL_ID", "amazon.nova-pro-v1:0"), os.getenv("AWS_REGION", "us-east-1"), enabled=bool(cfg.get("nova", {}).get("enabled", True)))
    log = RunLog()
    ...
    repos = [r for r in repos if scanning_cfg.get("include_archived", False) or not r.get("archived")]
    repos = [r for r in repos if scanning_cfg.get("include_forks", False) or not r.get("fork")]
    ...
    for repo_data in repos:
        ...
        try:
            ...
            log.add(Finding.now(owner, name, "repo", "error", "Repository scan failed", action="failed", details=f"{type(e).__name__}: {e}"))
