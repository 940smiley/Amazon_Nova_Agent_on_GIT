...
class GitHubClient:
    ...
    def get(self, path, params=None):
        ...
        if not r.ok:
            raise RuntimeError(
                f"GitHub GET {r.url} failed: {r.status_code} {r.text[:1000]}"
            )
        return r.json()
    ...
    def post(self, path, json=None):
        ...
        if not r.ok:
            raise RuntimeError(
                f"GitHub POST {r.url} failed: {r.status_code} {r.text[:1000]}"
            )
    ...
    def put(self, path, json=None):
        ...
        if not r.ok:
            raise RuntimeError(
                f"GitHub PUT {r.url} failed: {r.status_code} {r.text[:1000]}"
            )
        return r.json() if r.text else {}
    ...
    def list_user_repos(self, owner):
        return list(
            self.paginate(f"/users/{owner}/repos", {"per_page": 100, "sort": "updated"})
        )

    def dependabot_alerts(self, owner, repo, state="open"):
        return list(
            self.paginate(
    ...
    def issues(self, owner, repo):
        return list(
            self.paginate(
                f"/repos/{owner}/{repo}/issues", {"per_page": 100, "state": "open"}
            )
        )
    ...
    def pulls(self, owner, repo):
        return list(
            self.paginate(
                f"/repos/{owner}/{repo}/pulls", {"per_page": 100, "state": "open"}
            )
        )
    ...
    def pr_review_comments(self, owner, repo, number):
        return list(
            self.paginate(
    ...
    def combined_status(self, owner, repo, sha):
        return self.get(f"/repos/{owner}/{repo}/commits/{sha}/status")

    def merge_pr(self, owner, repo, number, strategy="squash"):
        return self.put(
            f"/repos/{owner}/{repo}/pulls/{number}/merge", {"merge_method": strategy}
        )
